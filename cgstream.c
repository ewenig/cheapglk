...
strid_t glk_stream_open_memory( char *buf, glui32 buflen, glui32 fmode, glui32 rock )
{
    ...
    if ( fmode != filemode_Read && fmode != filemode_Write && fmode != filemode_ReadWrite )
    {
        gli_strict_warning( "stream_open_memory: illegal filemode" );
        return NULL;
    }

    str = gli_new_stream( strtype_Memory, ( fmode != filemode_Write ), ( fmode != filemode_Read ), rock );
    if ( !str )
    {
...
strid_t glk_stream_open_file( fileref_t *fref, glui32 fmode, glui32 rock )
{
    stream_t *str;
    char modestr[16];
    FILE *fl;
    ...
    str = gli_new_stream( strtype_File, ( fmode == filemode_Read || fmode == filemode_ReadWrite ),
                          !( fmode == filemode_Read ), rock );
    if ( !str )
    {
...
strid_t glk_stream_open_memory_uni( glui32 *ubuf, glui32 buflen, glui32 fmode, glui32 rock )
{
    stream_t *str;
...
strid_t glk_stream_open_file_uni( fileref_t *fref, glui32 fmode, glui32 rock )
{
...
strid_t glk_stream_open_resource( glui32 filenum, glui32 rock )
{
    ...
    if ( res.chunktype == giblorb_ID_TEXT )
        isbinary = FALSE;
    else if ( res.chunktype == giblorb_ID_BINA || res.chunktype == giblorb_make_id( 'F', 'O', 'R', 'M' ) )
        isbinary = TRUE;
    else
        return 0; /* Unknown chunk type */

    str = gli_new_stream( strtype_Resource, TRUE, FALSE, rock );
    if ( !str )
    {
...
strid_t gli_stream_open_pathname( char *pathname, int writemode, int textmode, glui32 rock )
{
    char modestr[16];
    stream_t *str;
    FILE *fl;
    ...
    str = gli_new_stream( strtype_File, !writemode, writemode, rock );
    if ( !str )
    {
        fclose( fl );
        return NULL;
...
void glk_stream_set_position( stream_t *str, glsi32 pos, glui32 seekmode )
{
    ...
    case strtype_File:
        ...
        fseek( str->file, pos,
               ( ( seekmode == seekmode_Current ) ? 1 : ( ( seekmode == seekmode_End ) ? 2 : 0 ) ) );
        break;
    }
}
...
static glsi32 gli_get_char( stream_t *str, int want_unicode )
{
    ...
    case strtype_Resource:
        if ( str->unicode )
        {
            ...
            else
            {
                ...
                int flag = UTF8_DECODE_INLINE( &ch, ( str->bufptr >= str->bufend ),
                                               ( *( str->bufptr++ ) ), val0, val1, val2, val3 );
                if ( !flag )
                    return -1;
            }
            str->readcount++;
    ...
    case strtype_File:
        ...
        else
        {
            ...
            int res;
            glui32 ch;
            int flag = UTF8_DECODE_INLINE( &ch, ( res = getc( str->file ), res == -1 ),
                                           ( res & 0xFF ), val0, val1, val2, val3 );
            if ( !flag )
                return -1;
            str->readcount++;
            if ( !want_unicode && ch >= 0x100 )
...
static glui32 gli_get_buffer( stream_t *str, char *cbuf, glui32 *ubuf, glui32 len )
{
    if ( !str || !str->readable )
        return 0;
...
static glui32 gli_get_line( stream_t *str, char *cbuf, glui32 *ubuf, glui32 len )
{
    glui32 lx;
    int gotnewline;
