...
#define giblorb_native4( v )                                                                           \
    ( ( ( ( glui32 )( ( v )[3] ) ) & 0x000000ff ) | ( ( ( glui32 )( ( v )[2] ) << 8 ) & 0x0000ff00 ) | \
      ( ( ( glui32 )( ( v )[1] ) << 16 ) & 0x00ff0000 ) | ( ( ( glui32 )( ( v )[0] ) << 24 ) & 0xff000000 ) )
...
static giblorb_resdesc_t *giblorb_bsearch( giblorb_resdesc_t *sample, giblorb_resdesc_t **list, int len );
...
giblorb_err_t giblorb_create_map( strid_t file, giblorb_map_t **newmap )
{
    ...
    *newmap = NULL;
    ...
    chunks_size = 8;
    numchunks = 0;
    chunks = (giblorb_chunkdesc_t *)giblorb_malloc( sizeof( giblorb_chunkdesc_t ) * chunks_size );
    ...
    while ( nextpos < totallength )
    {
        ...
        {
            chunks_size *= 2;
            chunks = (giblorb_chunkdesc_t *)giblorb_realloc( chunks, sizeof( giblorb_chunkdesc_t ) * chunks_size );
        }
...
giblorb_err_t
giblorb_load_chunk_by_type( giblorb_map_t *map, glui32 method, giblorb_result_t *res, glui32 type, glui32 count )
{
    int ix;
...
giblorb_err_t giblorb_load_chunk_by_number( giblorb_map_t *map, glui32 method, giblorb_result_t *res, glui32 chunknum )
{
    ...
    case giblorb_method_Memory:
        if ( !chu->ptr )
        {
            ...
            glk_stream_set_position( map->file, chu->datpos, seekmode_Start );

            readlen = glk_get_buffer_stream( map->file, dat, chu->len );
            if ( readlen != chu->len )
                return giblorb_err_Read;
...
giblorb_err_t
giblorb_load_resource( giblorb_map_t *map, glui32 method, giblorb_result_t *res, glui32 usage, glui32 resnum )
{
...
giblorb_resdesc_t *giblorb_bsearch( giblorb_resdesc_t *sample, giblorb_resdesc_t **list, int len )
{
    int top, bot, val, res;
